...
- DE: Leistung je PV-Eingang: `solix_snapshot.py` reicht die MPPT-Kanäle (`solar_power_1..4`) als `pvWatts` durch. Per Option zeigt der PV-Wert die Eingänge einzeln statt der Summe ("438 · 204 W") — getrennt schaltbar für Dashboard, Menüleiste und abgedockte Leiste (wirkt jeweils auf einzeilige und Kompaktansicht). Hinweis: Feldnamen laut anker-solix-api, mangels Hardware unverifiziert — Solarbank 2 Pro/3 melden 4 Kanäle, Solarbank 2 Plus/AC 2, die erste Generation keine.
- EN: Per-PV-input power: `solix_snapshot.py` forwards the MPPT channels (`solar_power_1..4`) as `pvWatts`. An option shows the PV value as individual inputs instead of the total ("438 · 204 W") — separately toggleable for the dashboard, menu bar, and detached bar (each applies to single-line and compact views). Note: field names per anker-solix-api, unverified without hardware — Solarbank 2 Pro/3 report 4 channels, Solarbank 2 Plus/AC 2, the first generation none.
- DE: Behoben: Das Dashboard im Dropdown zeigte nach einer Einstellungsänderung gelegentlich den Graphen nicht (Layout kollabierte beim Appearance-Neuaufbau während des ersten Einblendens); erst erneutes Öffnen half.
- EN: Fixed: after a settings change the dropdown dashboard occasionally did not show the graph (layout collapsed during the appearance rebuild on first display); it only appeared after reopening.
